...
**GitHub Pages 배포 주소:**
```
https://enmanyproject.github.io/video/
```
...
- **📦 3MB 이하 최적화** (신규!):
  - 슬로우 적용 후 실제 영상 길이 계산
  - 비트레이트/FPS 자동 조정
- **🌟 프레임 보간 기술** (Alpha Blending):
  - **정상 구간**: 원본 프레임 그대로 (30fps 유지, 정속)
  - **슬로우 구간**: 두 프레임 사이에 중간 프레임을 생성하여 부드러운 슬로우 모션 구현
  - **보간 원리**:
    - 프레임 A와 프레임 B 사이에 중간 프레임들을 자동 생성
    - 중간 프레임 = 프레임 A × (1-α) + 프레임 B × α
    - 예: 0.5x 속도 → 2개 프레임 사이에 1개 중간 프레임 생성
    - 예: 0.25x 속도 → 2개 프레임 사이에 3개 중간 프레임 생성
  - **결과**: 단순 프레임 반복보다 훨씬 부드러운 슬로우 모션 효과
- **3구간 처리**:
  - 구간 1: 시작~슬로우구간 (정상 속도, 원본 fps)
  - 구간 2: 슬로우 구간 (프레임 보간으로 부드러운 슬로우)
  - 구간 3: 슬로우구간~끝 (정상 속도, 원본 fps)
- **FPS 선택**: 30fps, 60fps (권장)
- **품질 선택**: 3~10 Mbps
- **출력 포맷**:
  - **MP4** (localhost 전용 - FFmpeg 변환)
  - **WebM** (모든 환경 지원)
- **필터 적용**: 설정된 필터로 슬로우 모션 생성
- **진행률 표시**: 실시간 처리 상황

### ⚡ 고화질 비디오 (프레임 보간!)
**프레임 보간 기술**로 부드러운 고화질 영상 생성
- **목표 FPS**: 60fps, 90fps, 120fps (2배~4배 부드러움)
  - 원본 30fps 기준으로 프레임 보간
  - 120fps = 원본보다 4배 부드러운 재생
- **📦 3MB 이하 최적화** (신규!):
  - 고FPS 유지하며 비트레이트 자동 조정
  - 필요 시 FPS 자동 감소
- **🌟 프레임 보간 알고리즘**:
  - 원본 프레임 사이에 중간 프레임 자동 생성
  - Alpha Blending으로 자연스러운 움직임 구현
  - 재생 시간은 동일하게 유지 (느려지지 않음)
- **보간 품질**: 보통, 고품질, 최고품질 선택
- **비트레이트**: 8~15 Mbps (초고화질 지원)
- **출력 포맷**:
  - **MP4** (localhost 전용 - FFmpeg 변환)
  - **WebM** (모든 환경 지원)
- **필터 적용**: 설정된 필터로 고화질 비디오 생성
- **진행률 표시**: 실시간 처리 상황
...
### 🎵 오디오 합성
비디오에 배경음악이나 사운드 효과 추가
- **오디오 파일 선택**: MP3, WAV, OGG, M4A 등 지원
- **오디오 미리듣기**: 선택한 오디오 파일 미리 재생
- **원본 오디오 처리 옵션**:
  - **교체**: 원본 오디오 제거, 새 오디오만 사용
  - **믹싱**: 원본 + 새 오디오 혼합 (배경음악 추가)
  - **유지**: 원본 주, 새 오디오 배경 (30% 볼륨)
- **볼륨 조절**:
  - 새 오디오 볼륨: 0~200%
  - 원본 오디오 볼륨: 0~200%
- **Web Audio API**: 실시간 오디오 믹싱
- **WebM 출력**: Opus 오디오 코덱
- **진행률 표시**: 실시간 처리 상황
...
### 📦 3MB 최적화 활용 (신규!)
**메신저/SNS용 파일 생성 시 권장**
1. **자동 조정**: 체크 시 파일 크기 예측 후 최적 설정 자동 적용
2. **실시간 피드백**: 생성 전 "📦 3MB 최적화: 2.5Mbps, 24fps로 조정됨" 안내
3. **크기 경고**: 저장 완료 시 3MB 초과 시 ⚠️ 경고 표시
4. **지원 기능**: GIF 생성, 구간 잘라내기, 슬로우 모션, 역재생, 고화질 비디오

**최적화 알고리즘:**
- **GIF**: 프레임 수 × 너비 기반 자동 조정 (FPS/너비/품질)
- **비디오**: 영상 길이 기반 비트레이트 계산 `3MB ÷ 길이 = 목표 비트레이트`
- **슬로우 모션**: 슬로우 적용 후 실제 길이 계산하여 최적화
...
- **Web Audio API**: 실시간 오디오 믹싱 및 볼륨 조절
- **FFmpeg.wasm**: WebM → MP4 변환 (localhost 전용)
...
#### MP4 지원 (3단계 폴백 시스템)
**우선순위 1: FFmpeg 변환** (localhost 전용)
- **환경**: COOP/COEP 헤더 설정 가능한 localhost
- **코덱**: H.264 비디오 + AAC 오디오
- **품질**: CRF 23, medium preset
- **특징**: 가장 높은 호환성과 품질

**우선순위 2: 네이티브 MP4** (Safari 등)
- **환경**: MediaRecorder MP4 지원 브라우저
- **특징**: FFmpeg 로딩 실패 시 자동 폴백

**우선순위 3: WebM 폴백**
- **환경**: 모든 브라우저
- **코덱**: VP9/VP8 비디오 + Opus 오디오
- **특징**: 최종 폴백, 항상 지원됨

#### 고품질 인코딩
- **비트레이트**: 최대 15 Mbps (고화질)
- **FPS**: 최대 120fps (프레임 보간)
...
### MP4 변환이 작동하지 않음
→ **localhost 서버**(`npm start`)에서 실행 중인지 확인하세요.
→ GitHub Pages에서는 FFmpeg를 사용할 수 없어 네이티브 MP4 또는 WebM으로 저장됩니다.
→ Chrome/Firefox는 네이티브 MP4를 지원하지 않으므로 WebM으로 저장됩니다.

### WebM 파일이 재생되지 않음
→ **VLC Player, Chrome, Firefox** 등 WebM을 지원하는 플레이어를 사용하세요.
→ localhost에서 MP4 포맷을 선택하면 FFmpeg가 자동으로 변환합니다.
...
### v2.6 (2025-01-22) - 3MB 파일 크기 최적화 📦
- ✨ **3MB 이하 파일 크기 최적화 기능 추가**
  - 모든 비디오/GIF 생성 옵션에 체크박스 추가
  - 지능형 자동 조정: 파일 크기 예측 후 품질/FPS/비트레이트 최적화
  - 실시간 피드백: 생성 전 최적화 파라미터 안내 메시지
  - 파일 크기 경고: 저장 시 3MB 초과 시 ⚠️ 아이콘 및 메시지
- 📊 GIF 최적화: FPS, 너비, 품질 자동 조정
- 🎬 비디오 최적화: 비트레이트 자동 계산 (3MB ÷ 영상 길이)
- ⏱️ 슬로우 모션 특별 처리: 슬로우 적용 후 실제 길이 고려
- 🎯 적용 기능: GIF 생성, 구간 잘라내기, 슬로우 모션, 역재생, 고화질 비디오

### v2.5 (2025-11-20) - FFmpeg MP4 변환 🎬
- ✨ **FFmpeg.wasm 기반 MP4 변환 지원** (localhost 전용)
  - H.264 + AAC 코덱으로 고품질 MP4 생성
  - 3단계 폴백 시스템: FFmpeg → 네이티브 MP4 → WebM
  - 고화질, 슬로우 모션, 역재생 모두 MP4 지원
- 🔧 환경별 자동 감지 및 최적 포맷 선택
- 📊 변환 진행률 및 상태 메시지 표시

### v2.4 (2025-11-18) - 오디오 합성 🎵
- ✨ **오디오 합성 기능 추가**
  - 비디오에 배경음악/사운드 효과 추가
  - Web Audio API 기반 실시간 오디오 믹싱
  - 3가지 믹싱 모드: 교체, 믹싱, 유지
  - 개별 볼륨 조절 (0~200%)
  - 오디오 미리듣기 기능
- 🎧 고품질 오디오 처리 (GainNode, AudioContext)
- 📦 WebM + Opus 오디오 출력

### v2.3 (2025-11-18) - 고화질 비디오 ⚡
- ✨ **고화질 비디오 생성 기능**
  - 프레임 보간으로 60fps/90fps/120fps 출력
  - Alpha Blending 알고리즘으로 부드러운 중간 프레임 생성
  - 재생 시간 유지 (느려지지 않음)
  - 3단계 보간 품질: 보통, 고품질, 최고품질
  - 최대 15 Mbps 초고화질 비트레이트
- 🎨 필터 적용 및 실시간 진행률 표시
